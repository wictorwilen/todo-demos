 ...
 */
interface ITodoTeamsConnectorData {
    webhookUrl: string;
    user: string;
    appType: string;
    groupName: string;
...
/**
 * Implementation of the "TodoTeamsConnectorConnector" Office 365 Connector
 */
@ConnectorDeclaration(
    "/api/connector/connect",
    "/api/connector/ping"
)
@PreventIframe("/todoTeamsConnector/config.html")
export class TodoTeamsConnector implements IConnector {
    ...
    public Ping(req: Request): Array<Promise <void>> {
        ...
        try {
            this.connectors.push("/connectors",
                (this.connectors.getData("/connectors") as ITodoTeamsConnectorData[]).filter(((c) => {
                    return c.existing;
                })));
        } catch (error) {
        ...
        return (this.connectors.getData("/connectors") as ITodoTeamsConnectorData[]).map((connector, index) => {
                    ...
                    sections: [
                        {
                            ...
                            activityImage: "todo-teams.ngrok.io/assets/icon.png",
                            facts: [
                                {
                                    name: "Generator",
                                    value: "teams"
